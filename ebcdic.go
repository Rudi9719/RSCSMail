...
// EBCDIC (CP037) Translation Table stolen from Google
...
func generateEbcdicNote(from string, to []string, subject, fullSender string, body io.Reader, timestamp time.Time) (*bytes.Buffer, error) {
	...
	if fullSender != "" {
		writeEbcdicRecord(ebcdicBuf, []byte{0x40}, fmt.Sprintf("From: %s", fullSender))
	} else {
		writeEbcdicRecord(ebcdicBuf, []byte{0x40}, fmt.Sprintf("From: %s", from))
	}

	if subject != "" {
		writeEbcdicRecord(ebcdicBuf, []byte{0x40}, fmt.Sprintf("Subject: %s", subject))
	}

	writeEbcdicRecord(ebcdicBuf, []byte{0x40}, "")
